...
async function relay(data){
  const r=await fetch(`${cfg.server}/api/relay`,{
    method:'POST',headers:{'content-type':'application/json','authorization':auth},body:JSON.stringify({event:'gift',data})
  });
  if(!r.ok)console.error('Relay:',r.status,await r.text());
}
...
const connection=new TikTokLiveConnection(cfg.username,cfg.signApiKey?{signApiKey:cfg.signApiKey}:{});
connection.on(WebcastEvent.GIFT,d=>{
  const details=d.giftDetails??{};
  if(d.repeatEnd===false)return;
  relay({user:user(d.user),giftName:String(details.giftName??d.giftName??'Presente'),repeatCount:Math.max(1,Number(d.repeatCount??1)),diamondCount:Number(details.diamondCount??0)}).catch(console.error);
});
connection.on(ControlEvent.ERROR,e=>console.error('TikTok:',e?.message||e));
connection.on(ControlEvent.DISCONNECTED,()=>console.log('LIVE desconectada.'));

try{await connection.connect();console.log(`CONECTADO A @${cfg.username}`);console.log('Presentes sendo enviados para o jogo.');console.log('Deixe esta janela aberta.')}catch(e){console.error('Falha ao conectar:',e?.message||e);process.exit(1)}
