...
function loadConfig(){try{return JSON.parse(fs.readFileSync(CONFIG_FILE,'utf8'))}catch{return{server:'',username:'',adminPassword:'',signApiKey:''}}}
...
async function post(pathname,body){
  try{
    const r=await fetch(`${cfg.server}${pathname}`,{method:'POST',headers:{'content-type':'application/json','authorization':auth},body:JSON.stringify(body)});
    const text=await r.text();let d={};try{d=text?JSON.parse(text):{}}catch{d={error:text}}
    if(!r.ok){console.error(`ERRO ${r.status}:`,d.error||d);return null}return d;
  }catch(e){console.error('ERRO DE CONEXAO COM O JOGO:',e?.message||e);return null}
}
async function sendRelay(event,data){return post('/api/relay',{event,data})}
async function rememberGift(gift){if(!gift?.id||!gift?.name)return;const r=await sendRelay('giftCatalog',{gifts:[gift]});if(r?.count!=null)console.log(`🧠 Catalogo atualizado | ${gift.name} | ID ${gift.id} | total salvo ${r.count}`)}
async function relayGift(data){
  ...
  const body=await sendRelay('gift',data);if(!body)return;const q=Math.max(1,Number(data.repeatCount)||1);
...
async function relayComment(data){
  const who=data.user?.nickname||data.user?.uniqueId||'Usuario';
  console.log(`💬 COMENTARIO RECEBIDO | ${who}: "${data.comment}"`);
  const body=await sendRelay('comment',data);
  if(!body){console.log('⚠️ Comentario nao chegou ao servidor do jogo.');return}
  if(body.matched)console.log(`✅ VOTO POR COMENTARIO | ${who} → ${body.option?.name||'opcao'} | total ${body.option?.count??'?'}`);
  else if(body.disabled)console.log('⚠️ VOTO NAO CONTADO | Votos por comentario estao DESATIVADOS no Admin. Ative e clique em APLICAR E SALVAR.');
  else console.log(`⚠️ VOTO NAO CONTADO | comentario "${data.comment}" nao corresponde exatamente ao nome de nenhuma opcao.`);
}
...
const connection=new TikTokLiveConnection(cfg.username,{...(cfg.signApiKey?{signApiKey:cfg.signApiKey}:{}),enableExtendedGiftInfo:false});
function resolveGift(d){const details=d?.giftDetails??{},giftId=str(d?.giftId??details?.giftId??d?.extendedGiftInfo?.id),giftType=Number(details?.giftType??d?.giftType??0),giftName=str(details?.giftName)||str(d?.giftName)||str(d?.extendedGiftInfo?.name)||`Presente ID ${giftId||'desconhecido'}`,giftImage=str(d?.extendedGiftInfo?.pictureUrl)||str(d?.extendedGiftInfo?.image?.urlList?.[0])||str(details?.image?.urlList?.[0])||str(details?.image?.url_list?.[0]);return{giftId,giftType,giftName,giftImage}}
connection.on(WebcastEvent.GIFT,d=>{const g=resolveGift(d),repeatEnd=d?.repeatEnd===true,repeatCount=Math.max(1,Number(d?.repeatCount??1)||1);if(g.giftType===1&&!repeatEnd)return;relayGift({user:user(d?.user),giftId:g.giftId,giftType:g.giftType,giftName:g.giftName,giftImage:g.giftImage,repeatCount,diamondCount:Number(d?.giftDetails?.diamondCount??d?.diamondCount??0)}).catch(e=>console.error('Falha ao enviar presente:',e?.message||e))});
connection.on(WebcastEvent.CHAT,d=>{
  const comment=str(d?.comment??d?.message??d?.content??d?.text);
  if(!comment)return;
...
try{
  ...
  console.log('✅ Comentarios estao sendo monitorados e agora aparecem no console para diagnostico.');
  console.log('Para votar por comentario, ative a opcao no Admin e clique em APLICAR E SALVAR.');
