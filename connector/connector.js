...
async function relay(data){
  const r=await fetch(`${cfg.server}/api/relay`,{method:'POST',headers:{'content-type':'application/json','authorization':auth},body:JSON.stringify({event:'gift',data})});
  const body=await r.json().catch(async()=>({error:await r.text().catch(()=>String(r.status))}));
  if(!r.ok){console.error(`ERRO RELAY ${r.status}:`,body.error||body);return}
...
connection.on(WebcastEvent.GIFT,d=>{
  ...
  // Presentes de streak (giftType 1) emitem eventos parciais. Contamos apenas o evento final.
  ...
  relay({
    user:user(d.user),
    giftId:String(d.giftId??details.giftId??''),
    giftType,
    giftName,
    giftImage,
    repeatCount,
    diamondCount:Number(details.diamondCount??d.extendedGiftInfo?.diamondCount??0)
...
try{
  ...
  console.log('Contagem: presente normal = +1; combo/streak = +quantidade final do combo.');
